...
float snn_infer(const float *x_raw, snn_stats_t *st)
{
    ...
    for (int t = 0; t < SNN_T; ++t) {
        ...
        /* ---- readout: non-spiking integrator (beta=1, threshold=inf) ---- */
        float o = snn_out_b[0];
        for (int j = 0; j < SNN_H; ++j) {
...
float snn_infer_dense(const float *x_raw, snn_stats_t *st)
{
    ...
    for (int t = 0; t < SNN_T; ++t) {
        ...
        float o = snn_out_b[0];
        for (int j = 0; j < SNN_H; ++j) { o += snn_out_w[j] * s2[j]; macs++; }
        acc += o;
    }
